...
while True:
    ...
    TEMP= "{:.1f}°C".format(round(temps['cpu_thermal'][0].current,2))
    ...
    draw.text((x+90, top),             HOSTNAME,       font=font, fill=255)
    draw.text((x, top+FONTSIZE),    CPU,      font=font, fill=255)
    draw.text((x+70,top+FONTSIZE),  TEMP,     font=font, fill=255)
    #draw.text((x+55, top+FONTSIZE),  MemUsage, font=font, fill=255)
